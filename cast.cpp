...
private:
  void loadWorld(string levelFileName) {
    ...
    int index = 0;
    world.cells =
        (char *)calloc(world.size.width * world.size.height, sizeof(char));
...
float dtor(float angle) { return ((float)(PI / 180.0f) * angle); }
...
int main(int argc, char **argv) {
  ...
  while (i != 27) {
    ...
    if (i == 'w') {
      settings.player.pos =
          vplus(settings.player.pos, times(settings.player.dir, 0.5f));
    } else if (i == 's') {
      settings.player.pos =
          vminus(settings.player.pos, times(settings.player.dir, 0.5f));
    } else if (i == 'a') {
      settings.player.pos =
          vplus(settings.player.pos,
                times(rot(dtor(-90)), times(settings.player.dir, 0.5f)));
    } else if (i == 'd') {
      settings.player.pos =
          vplus(settings.player.pos,
                times(rot(dtor(90)), times(settings.player.dir, 0.5f)));
...
void castAllColumns(Hit *hits, const Settings &settings) {
  ...
  V planeStart = vplus(
      player.pos, vplus(playerToPlane, times(planeDir, camera.width / 2.0f)));

  for (int x = 0; x < screen.size.width; ++x) {
    V rayOnPlane =
        vplus(planeStart,
              times(times(rot(dtor(180.0f)), planeDir),
                    camera.width * (1.0f - (float)x / screen.size.width)));
    ...
    for (float distance = 0.01f; distance < 100.f && !hit; distance += 0.001f) {
      ...
      if (int(check.x) >= 0 && int(check.x) < world.size.width &&
        ...
        char cell =
            world.cells[int(check.y) * int(world.size.w) + int(check.x)];
...
void render(const Settings &settings) {
  ...
  for (int y = 0; y < settings.screen.size.height; ++y) {
    for (int x = 0; x < settings.screen.size.width; ++x) {
      if (!drawScreenFrame(x, y, settings.screen.size.width,
                           settings.screen.size.height) &&
...
void title(string message, const Settings &settings) {
  ...
  cout << maze(w) << endl;
  cout << maze(w) << endl;
  cout << maze(w2 - message.length() / 2 - 2) << "  " << string(10, ' ') << "  "
       << maze(w2 - message.length() / 2 - 2) << endl;
  cout << maze(w2 - message.length() / 2 - 2) << "  " << message << "  "
       << maze(w2 - message.length() / 2 - 2) << endl;
  cout << maze(w2 - message.length() / 2 - 2) << "  " << string(10, ' ') << "  "
       << maze(w2 - message.length() / 2 - 2) << endl;
  cout << maze(w) << endl;
...
void info(string message) { cout << "i: " << message << endl; }

void error(string message) { error(message, ""); }
